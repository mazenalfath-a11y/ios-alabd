class PDFEditor {
    ...
    async renderPages() {
        ...
        for (let i = 1; i <= this.totalPages; i++) {
            ...
            // Fix for High DPI screens (dpr is already defined above)
            annotationLayer.width = Math.floor(viewport.width * dpr);
            annotationLayer.height = Math.floor(viewport.height * dpr);
            annotationLayer.style.width = Math.floor(viewport.width) + "px";
            annotationLayer.style.height = Math.floor(viewport.height) + "px";
            
            // Scale the context to ensure drawing operations use logical pixels
    ...
    getEventPosition(event, canvas) {
        ...
        // مع دعم High DPI، الحجم الفعلي للكانفس أكبر من الحجم الظاهري بـ DPR
        // لذلك نحتاج للقسمة على DPR للحصول على الإحداثيات المنطقية (Logical Pixels)
        const dpr = window.devicePixelRatio || 1;
